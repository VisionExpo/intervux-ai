...
def validate_signature():
    ...
    for table_name, expected_columns in SCHEMA_SIGNATURE.items():
        if table_name not in Base.metadata.tables:
            errors.append(f"Table '{table_name}' defined in SCHEMA_SIGNATURE but not found in SQLAlchemy models.")
            continue
        ...
        if missing_columns:
            errors.append(f"Table '{table_name}' is missing columns in SQLAlchemy models: {missing_columns}")
            
    # 3. Check for tables in models that are NOT in signature (Optional/Warning)
    # This helps identify if new tables were added but forgotten in signature
    model_tables = set(Base.metadata.tables.keys())
    signature_tables = set(SCHEMA_SIGNATURE.keys())
    
    untracked_tables = model_tables - signature_tables
    # Ignore internal/meta tables if any
