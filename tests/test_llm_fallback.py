...
@pytest.mark.asyncio
async def test_run_json_task_falls_back_when_primary_is_unavailable(monkeypatch):
    calls: list[str] = []

    def fake_call_provider(provider: str, prompt: str, *args, **kwargs) -> str:
    ...
    payload, provider = lb._run_json_task("{}", MockSchema, temperature=0.1)
...
@pytest.mark.asyncio
async def test_run_json_task_does_not_fallback_for_non_retriable_error(monkeypatch):
    calls: list[str] = []
    ...
    with pytest.raises(RuntimeError, match="response schema mismatch"):
        lb._run_json_task("{}", MockSchema, temperature=0.1)

    assert calls == ["gemini"]


@pytest.mark.asyncio
async def test_prewarm_uses_fallback_provider_when_primary_fails(monkeypatch):
    calls: list[str] = []
