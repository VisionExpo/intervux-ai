...
export const AdminKPIRow: React.FC<AdminKPIRowProps> = ({ evaluationData }) => {
  ...
  const kpis: KPI[] = [
    ...
    },
    {
      ...
      value: d?.system_health ? `${d.system_health.active_interview_sessions}` : '—',
      trend: d?.system_health ? `Queue: ${d.system_health.queue_length}` : 'Stable',
      trendColor: 'text-tertiary',
    },
    {
      label: 'AI Accuracy',
      value: d?.model_quality ? `${(d.model_quality.accuracy * 100).toFixed(1)}%` : '—',
      trend: d?.model_quality ? `Consistency: ${(d.model_quality.consistency_score * 100).toFixed(0)}%` : '',
      trendColor: 'text-primary',
    },
    {
      label: 'Error Rate',
      value: d?.performance ? `${(d.performance.error_rate * 100).toFixed(2)}%` : '—',
      bar: d?.performance ? Math.min(d.performance.error_rate * 1000, 100) : 0,
    },
    {
      label: 'Latency (p95)',
      value: d?.performance?.latency ? `${d.performance.latency.p95.toFixed(0)}ms` : '—',
      trend: d?.performance?.latency ? `p99: ${d.performance.latency.p99.toFixed(0)}ms` : '',
      trendColor: 'text-primary',
    },
    {
      label: 'Daily AI Spend',
      value: d?.cost ? `$${d.cost.daily_ai_spend.toFixed(2)}` : '—',
